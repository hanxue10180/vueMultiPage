		...
		<div slot="body">
			...
			<div class="btable">
			  	...
			  	<div id="toolbar" class="btn-group pull-right">
		        	...
		        	<button v-if="num!=1 && state!=2 " type="button" class="btn btn-default"  v-on:click="adds" allow="szywedit">收款 </button>
		       		<button v-if="num==1" type="button" class="btn btn-default" v-on:click="addRecord" allow="szywedit">追加收支记录</button>
	        	</div>
	       	</div>
	       	<div class="hr-line-dashed" ></div>
	       	<div class="fline" style="height:35px;">
	       		<formg>
	       			<span slot="tl">（收）计划合计（元）</span>
	       			<span slot="ct" >{{ total1 }}</span>
	       		</formg>
	       		<formg>
	       			<span slot="tl">（收）实际合计（元）</span>
	       			<span slot="ct" >{{ ptotal1}}</span>
	       		</formg>
	       		<formg>
	       	...
	       	<div class="fline" style="height:35px;" >
	       		<formg>
	       			<span slot="tl">（支）计划合计（元）</span>
	       			<span slot="ct" >{{ total2 }}</span>
	       		</formg>
	       		<formg>
	       			<span slot="tl">（支）实际合计（元）</span>
	       			<span slot="ct" >{{ ptotal2 }}</span>
	       		</formg>
	       		<formg>
					<span slot="tl">差额（元）</span>
	...
	export default{
		...
		methods:{
			...
			adds(){
				...
				if(dataArr[0].expensesReceiptType == 1){
					toast.err('计划类型为收不能进行收款');
					return false ;
				}
				var _info = {
					info1:JSON.stringify($api.vueToObj(this.info1)),
			...
			edit(){
				...
				if(!dataArr.length){
					...
					var _info = {
						...
						info2:JSON.stringify($api.vueToObj(this.info2)),
						id:dataArr[0].id,
						payId:dataArr[0].payId,
						flag:dataArr[0].expensesReceiptType==1 ? 2 : 1
					}
					this.$router.push({
						path:'/depositEdit',
						query:_info
			...
			getList(){
				...
				$api.table('#mytab',{
					...
					onLoadSuccess:function(data){
						var _list = data.obj ;
						let [t1,t2,t3,t4,t5,t6] = [0,0,0,0,0,0];
						for(var i=0;i<_list.length;i++){
							_list[i].operateAmount = _list[i].operateAmount ? _list[i].operateAmount : 0 ;
							//收
							if(_list[i].expensesReceiptType == 1){
								t1 += _list[i].expectFee ;
								t2 += _list[i].operateAmount ;
							}
							//支
							else{
								t4 += _list[i].expectFee ;
								t5 += _list[i].operateAmount ;
							}							
						}
						t3 = t2 - t1 ;
						t6 = t5 - t4 ;
						self.total1 = t1.toFixed(2) ;
						self.ptotal1 = t2.toFixed(2) ;
						self.chae1 = t3.toFixed(2) ;
						self.total2 = t4.toFixed(2) ;
						self.ptotal2 = t5.toFixed(2) ;
						self.chae2 = t6.toFixed(2) ;
					}
				})
